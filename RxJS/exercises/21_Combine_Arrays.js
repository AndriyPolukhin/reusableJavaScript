/**
 * COMBINE ARRAYS
 * Ex.21: Combine videos and bookmarks by index
...
for (counter = 0; counter < Math.min(videos.length, bookmarks.length); counter++) {
  ...
  // console.log(Math.min(videos.length, bookmarks.length));
  const videoId = videos[counter].id;
  const bookmarkId = bookmarks[counter].id;
